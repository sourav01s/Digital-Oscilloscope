# STM32L072CZ Oscilloscope

A DIY oscilloscope: an STM32L072CZ (LoRa/SX1276 board) samples an analog
signal on its ADC and streams it over SPI to an ESP32, which serves a live
waveform display to any browser on the network. Everything on the STM32
side is written bare-metal .
...
- STM32L072CZ + SX1276 LoRa board (e.g. B-L072Z-LRWAN1 or similar)
...
**STM32 side:**
1. Put `adc.h`, `spi.h` in `Core/Inc/`
2. Put `adc.c`, `spi.c`, and `test_spi_main.c` (as your `main.c`) in `Core/Src/`
3. Build and flash via STM32CubeIDE or your Makefile toolchain

**ESP32 side:**
1. Install `ESPAsyncWebServer` and `AsyncTCP` libraries (Arduino Library Manager)
2. Edit `WIFI_SSID` / `WIFI_PASS` in `esp32_spi_web.ino`
3. Flash via Arduino IDE
4. Open Serial Monitor (115200 baud) to get the assigned IP address
5. Visit `http://<that-ip>/` in a browser on the same network
